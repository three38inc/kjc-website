<div class="row">
    <div class="col-lg-5">
        <h4 class="mainheading"><i class="fa fa-bookmark"></i>
            <span>&nbsp;Media <strong>  Room</strong></span>
        </h4><br>
    </div>
    <div class="col-lg-1 col-md-2 hidden-xs hidden-sm"></div>
    <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
        <div class="filters filter-button-course">
            
            
        </div>
    </div>
</div>

<div class="row">

    <div class="grid" id="course">
        <div class="col-lg-4 col-md-4 science"></div>
        <div class="col-lg-4 col-md-4 science"></div>
        <div class="col-lg-4 col-md-4 science"></div><br>

       

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="news.php">
                <div class="courseTitle color-4">
                    <i class="fa fa-file c4" style="float: left;"></i>
                    <h2>News</h2>
                    <p>Regular updates of the activities and events of the college..</p>
                </div>  
            </a>
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="in_house_publications.php">
                <div class="courseTitle color-5">
                    <i class="fa fa-home c5" style="float: left;"></i>
                    <h2>In-house publications</h2>
                    <p>Kristu Jayanti College Media Room also manages in-house multi-media platforms..</p>
                </div>  
            </a>
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="photographs.php">
                <div class="courseTitle color-3">
                    <i class="fa fa-camera-retro c3" style="float: left;"></i>
                    <h2>Photographs</h2>
                    <p>Our dedicated photographers provide a visual experience about the college..</p>
                </div>  
            </a>
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="press_releases.php">
                <div class="courseTitle color-7">
                    <i class="fa fa-newspaper-o c7" style="float: left;"></i>
                    <h2>Press releases</h2>
                    <p>We would like to publish the events and activities beyond the boundaries..</p>
                </div>  
            </a>
        </div>

        
        
      
        
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="in_the_news.php" target="_blank">
                <div class="courseTitle color-1">
                    <i class="fa fa-file-text c1" style="float: left;"></i>
                    <h2>In the news</h2>
                    <p>Kristu Jayanti College activities and other initiatives are carried..</p>
                </div>  
            </a>
        </div>
        

        
        
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="social_media.php">
                <div class="courseTitle color-7">
                    <i class="fa fa-share-square-o c7" style="float: left;"></i>
                    <h2>Social Media</h2>
                    <p>The social media hub of Kristu Jayanti College provides an overview..</p>
                </div>  
            </a>
        </div>
        
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="kjc-television.php">
                <div class="courseTitle color-1">
                    <i class="fa fa-television c1" style="float: left;"></i>
                    <h2>KJC Television</h2>
                    <p>Kristu Jayanti Television: an offline TV programming and production started..</p>
                </div>  
            </a>
        </div>

        <!--<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="#">
                <div class="courseTitle color-2">
                    <i class="fa fa-flask c2" style="float: left;"></i>
                    <h2>M.Sc.</h2>
                    <p>Biotechnology</p>
                </div>  
            </a>
        </div>


        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="Faculty-Sciences/department-lifescience/msc_micro.php">
                <div class="courseTitle color-3">
                    <i class="fa fa-gg c3" style="float: left;"></i>
                    <h2>M.Sc</h2>
                    <p>Microbiology</p>
                </div>  
            </a>
        </div>
        
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 science" data-category="science">
            <a href="Faculty-Sciences/department-lifescience/ls_research.php">
                <div class="courseTitle color-2">
                    <i class="fa fa-search c2" style="float: left;"></i>
                    <h2>Research Center</h2>
                    <p>Research Center - Biotechnology</p>
                </div>  
            </a>
        </div>-->


        <!--<div class="col-lg-4 col-md-4 arts"></div>
        <div class="col-lg-4 col-md-4 arts"></div>
        <div class="col-lg-4 col-md-4 arts"></div>
        <div class="col-lg-12 arts" data-category="science">
            <h4>Department Of Humanities</h4><br/>        
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="faculty-humanities/english/ma_english_literature.php">
                <div class="courseTitle color-3">
                    <i class="fa fa-book c3" style="float: left;"></i>
                    <h2>MA</h2>
                    <p>English Literature</p>
                </div>           
            </a>
        </div>
        
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="faculty-humanities/humanities/ba_hep.php">
                <div class="courseTitle color-1">
                    <i class="fa fa-book c1" style="float: left;"></i>
                    <h2>BA</h2>
                    <p>History, Economics, Political Science</p>
                </div>           
            </a>
        </div>


        <div class="col-lg-12 arts" data-category="science">
            <h4>Department Of Journalism &amp; Mass-Communication</h4><br/>        
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="faculty-humanities/journalism/ba_htj.php">
                <div class="courseTitle color-2">
                    <i class="fa fa-book c2" style="float: left;"></i>
                    <h2>BA</h2>
                    <p>History, Tourism, Journalism</p>
                </div>           
            </a>
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="faculty-humanities/journalism/ma_journalism_mass_communication.php">
                <div class="courseTitle color-4">
                    <i class="fa fa-book c4" style="float: left;"></i>
                    <h2>MA</h2>
                    <p>Mass Communication &amp; Journalism</p>
                </div>           
            </a>
        </div>
        
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="faculty-humanities/journalism/ba_jpcs.php">
                <div class="courseTitle color-5">
                    <i class="fa fa-book c5" style="float: left;"></i>
                    <h2>BA</h2>
                    <p>Journalism, Psychology, Computer Science</p>
                </div>           
            </a>
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="faculty-humanities/journalism/ba_jpoleng.php">
                <div class="courseTitle color-6">
                    <i class="fa fa-book c6" style="float: left;"></i>
                    <h2>BA</h2>
                    <p>Journalism, Political Science, English Literature</p>
                </div>           
            </a>
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="faculty-humanities/journalism/ba_jpeng.php">
                <div class="courseTitle color-7">
                    <i class="fa fa-book c7" style="float: left;"></i>
                    <h2>BA</h2>
                    <p>Journalism, Psychology, English Literature</p>
                </div>           
            </a>
        </div>


        <div class="col-lg-12 arts" data-category="science">
            <h4>Department Of Social Work</h4><br/>        
        </div>


        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="faculty-humanities/socialwork/master_socialwork.php">
                <div class="courseTitle color-1">
                    <i class="fa fa-book c1" style="float: left;"></i>
                    <h2>MSW</h2>
                    <p>Master of Social Work</p>
                </div>           
            </a>
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="Faculty-Humanities/socialwork/research_activities.php">
                <div class="courseTitle color-3">
                    <i class="fa fa-book c3" style="float: left;"></i>
                    <h2>Research Center</h2>
                    <p>Research Center - Social Work.</p>
                </div>           
            </a>
        </div>


        <div class="col-lg-12 arts" data-category="science">
            <h4>Department Of Psychology</h4><br/>        
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 arts" data-category="arts">
            <a href="faculty-humanities/psychology/msc_program.php">
                <div class="courseTitle color-2">
                    <i class="fa fa-book c2" style="float: left;"></i>
                    <h2>M.Sc</h2>
                    <p>Psychology</p>
                </div>           
            </a>
        </div>
        
        

        <div class="col-lg-4 col-md-4 commerce"></div>
        <div class="col-lg-4 col-md-4 commerce"></div>
        <div class="col-lg-4 col-md-4 commerce"></div>
        
        <div class="col-lg-12 commerce" data-category="commerce">
            <h4>Department of Commerce</h4><br/>        
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/commerce-pg/mcom.php">
                <div class="courseTitle color-5">
                    <i class="fa fa-line-chart c5" style="float: left;"></i>
                    <h2>M.Com</h2>
                    <p>Master of Commerce</p>
                </div> 
            </a>          
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/commerce-pg/mcom_fa.php">
                <div class="courseTitle color-6">
                    <i class="fa fa-line-chart c6" style="float: left;"></i>
                    <h2>M.Com</h2>
                    <p>Financial Analysis</p>
                </div> 
            </a>          
        </div>
        
        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/commerce-pg/mcom_fa_cgma.php">
                <div class="courseTitle color-6">
                    <i class="fa fa-line-chart c6" style="float: left;"></i>
                    <h2>M.Com</h2>
                    <p>Financial Analysis with CGMA</p>
                </div> 
            </a>          
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/economics/ma_economics.php">
                <div class="courseTitle color-7">
                    <i class="fa fa-line-chart c7" style="float: left;"></i>
                    <h2>MA</h2>
                    <p>Economics</p>
                </div> 
            </a>          
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/commerce/bcom.php">
                <div class="courseTitle color-2">
                    <i class="fa fa-line-chart c2" style="float: left;"></i>
                    <h2>B.Com</h2>
                    <p>Bachelor of Commerce</p>
                </div> 
            </a>          
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/commerce/bcom_professioal.php">
                <div class="courseTitle color-3">
                    <i class="fa fa-line-chart c3" style="float: left;"></i>
                    <h2>B.Com</h2>
                    <p>Professional</p>
                </div> 
            </a>          
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/commerce/bcom_integrated_bcom_acca.php">
                <div class="courseTitle color-4">
                    <i class="fa fa-line-chart c4" style="float: left;"></i>
                    <h2>B.Com</h2>
                    <p>ACCA Integrated</p>
                </div> 
            </a>          
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/commerce/bcom_tourism.php">
                <div class="courseTitle color-5">
                    <i class="fa fa-line-chart c5" style="float: left;"></i>
                    <h2>B.Com</h2>
                    <p>Tourism</p>
                </div> 
            </a>          
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="science">
            <a href="faculty-commerce-management/commerce/commerce_value_added_courses.php">
                <div class="courseTitle color-1">
                    <i class="fa fa-line-chart c1" style="float: left;"></i>
                    <h2>Certification Course</h2>
                    <p><span style="font-size: 12px;">NSDC On BFSI (Banking Financial Services and Insurance)</span></p>
                </div>  
            </a>
        </div>

        <div class="col-lg-12 commerce" data-category="commerce">
            <h4>Department of Management</h4><br/>        
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/management/bba.php">
                <div class="courseTitle color-1">
                    <i class="fa fa-line-chart c1" style="float: left;"></i>
                    <h2>BBA</h2>
                    <p>Bachelor of Bussiness Administration</p>
                </div> 
            </a>          
        </div>

        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 commerce" data-category="commerce">
            <a href="faculty-commerce-management/management/pg_diploma.php">
                <div class="courseTitle color-6">
                    <i class="fa fa-line-chart c6" style="float: left;"></i>
                    <h2>PG Diploma</h2>
                    <p>1 Year PG Diploma for Working Executives</p>
                </div> 
            </a>          
        </div>-->
        


    </div>

</div>











