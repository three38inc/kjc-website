<br><br/>
<center><h4 class="mainheading" style="background-position: bottom center;">
    <span style="color: #fff;">&nbsp;Kristu Jayanti <strong> College of  Arts, Science And Commerce</strong></span>
    </h4></center>
<div class="row">

    <div class="col-lg-4 deanary" id="falSci">
        <div class="iconDiv">
            <i class="fa fa-folder-open-o" aria-hidden="true"></i>
        </div>
        <h3>Faculty of Science</h3>
        <p>Faculty of Science is working at the cutting-edge of developments, providing an environment in which students can draw ..</p>
        <a href="Faculty-Sciences/index.php">Know More</a>
    </div>

    <div class="col-lg-4 deanary" id="falCom">
        <div class="iconDiv">
            <i class="fa fa-folder-open-o" aria-hidden="true"></i>
        </div>
        <h3>Faculty Of Commerce</h3>
        <p>Faculty of Commerce provides an educational experience that extends beyond the academic excellence, generating opportunities ..</p>
        <a href="Faculty-Commerce-Management/index.php">Know More</a>
    </div>

    <div class="col-lg-4 deanary" id="falHum">
        <div class="iconDiv">
            <i class="fa fa-folder-open-o" aria-hidden="true"></i>
        </div>
        <h3>Faculty Of Humanities</h3>
        <p>The Faculty of Humanities ensures the highest quality of teaching, providing an aesthetic exploration of Humanities from the literary..</p>
        <a href="Faculty-Humanities/index.php">Know More</a>
    </div>

</div>

